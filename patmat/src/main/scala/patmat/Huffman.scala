...
object Huffman {
  ...
  /**
    ...
    */
  def until(
      criteria: List[CodeTree] => Boolean,
      combineFun: List[CodeTree] => List[CodeTree]
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def _decode(subtree: CodeTree, bits: List[Bit]): List[Char] =
      subtree match {
        case Leaf(chars, _) =>
          if (bits.isEmpty) List(chars) else chars :: _decode(tree, bits)
        case Fork(left, r, _, _) =>
          if (bits.head == 0) _decode(left, bits.tail)
          else _decode(r, bits.tail)
      }
    _decode(tree, bits)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def lookup(tree: CodeTree)(c: Char): List[Bit] = tree match {
      case Leaf(c, _) => List()
      case Fork(l, r, _, _) =>
        if (chars(l).contains(c)) List(0) ::: lookup(l)(c)
        else List(1) ::: lookup(r)(c)
    }
