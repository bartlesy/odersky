...
object Anagrams {
   ...
   */
  // def combinations(occurrences: Occurrences): List[Occurrences] = occurrences.toSet.subsets.toList.map(_.toList)
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    def _combinations(in: List[Char]): Seq[String] =
      for {
        len <- 1 to in.length
        combinations <- in combinations len
      } yield combinations.mkString

    List() :: _combinations(occurrences.map(x => x._1.toString * x._2).flatten).map(wordOccurrences).toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x
    .map(x => (x._1, x._2 - y.toMap.getOrElse(x._1, 0)))
    .sortBy(_._2)
    .dropWhile(_._2 == 0)
